#include<iostream>
using namespace std;
//udate in Range (l,r) by x , for this 
//we use lazy propagetion segment tree

//this code for(sum Range Query) find sum in range (l,r)
...
void update(int *a,int *tree,int *lazy,int s,int e,
	int treeNode,int l,int r,int value){

	if(s>e)return;

	//first update lazy propagation tree if need
	if(lazy[treeNode]!=0){
	...
	if(s>=l && e<=r){
		tree[treeNode]+=value;
		if(s!=e){
			lazy[treeNode*2]+=value;
			lazy[treeNode*2+1]+=value;
		}
		return;
	}
